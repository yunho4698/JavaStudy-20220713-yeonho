...
public class StudentService {
	...
	public boolean showMainMenu() {
		...
		select = inputSelect();
		//select 두번 입력 받아서 오류 났음
		// inputSelect 로 넘어가서 하니깐 밑에 버퍼 줄 필요 없음
		// return scanner.nextLine();  이거
		if(select.equals("1")) {
			Student student = addStudent();
			if(student != null) {
				System.out.println(student.toString()); //객체가 가지고 있는 정보를 한눈에 보여주기 위해서 toString을 사용
			}
		...
		System.out.println();
		return true; //메인 while안에 것이 계속 실행되도록 true 를 줌, 위에 false 는 while을 멈추기 위해 주는 return 값.
	}
	...
	private Student searchStudentByStudentName(String searchtName) {
		for(int i = 0; i < studentArray.length; i++) {
			if(studentArray[i] != null) {
				String studentName = studentArray[i].getStudentName();
				if(searchtName.equals(studentName)) {
					return studentArray[i];
				}
			}
		}
